/*
	...
	Moto-Mast Firmware Ver. 1.02				* * * UPDATE DISPLAY ON APPROX. LINE 167 * * * 
*/
...
void setup() {
	...
	*/
	...
	tft.setTextSize(0);
	tft.setCursor(10, 20);
	tft.println(F("Moto-Mast Firmware Ver. 1.02"));				//	<========================================================  Update this when finished with editing
	delay(2000);
...
byte SerialCheck() {
	while (Serial.available() > 0) {
		...
		if (Serial.read() == '\n') {
			...
			// Let's echo the data back to the screen for a double check
			Serial.println((String)"SetHt = " + SetHeight + "  SetSpd = " + SetSpeed);
