...
int mPWM = 0;
float SlowTrigger = 1.5;	// Distance to target in feet where speed show be slowed down.
...
int SlowSpeed = 25;				// Planned slow speed
...
void setup() {
	...
	*/
	...
	// For test, let's start out at 15 feet regardless of position.  NEED TO REMOVE THIS LATER FOR REAL USE ***************************************************************************************************************
	ShaftCount = 19;
...
//	Serial.println(millis());		// Put in to check interrupt processing time.  Currently less than 1 mSec.
	...
	if (Direction) {				// Going Up
		...
		if (RunHeight >= (SetHeight-0.26) || RunHeight >= 26.74) {			// Want to stop motor immediately when it gets within 1 shaft rotation of target
			...
			RunActive = LOW;
			RunningSlow = LOW;			// Reset system for normal speed
			DrawLights();
		}
...
byte RunActions() {
	...
	if ((!ButtonPushed[2]) & ActiveScreen[4]) {		// Button 2 is Auto-Up
		...
		DisplayRun();
		ResetButtons();
	//	Serial.println(F("Auto-Up is finished."));
	}
...
void RunScenario() {	// Uses as inputs the global variables Direction, SetSpeed, SetHeight
	...
	SoftDelay(1000);	// Give things a a second to stabilize in off state
	VOn48 = LOW;
...
void DisplayStatusHeight(int StatusHeight) {
	...
	tft.fillRect(155, 110, 200, 25, ILI9341_NAVY);  // Erase old count first.  Note that args are (x, y, len x, len y)
...
int CalculatePWM(int RequestedSpeed) {
	...
	PWMtoReturn = PWMtoSpeedMapping[RequestedSpeed / 5];
	return PWMtoReturn;
}
...
*/
	...
	if (ms == mset) {		// motor is being turned on and speed should be ramped up
		...
		for (int i = 50; i <= mPWM; i++) {		// Value of 50 is about lowest PWM where motor is still running.  So effectively starts at 0 speed.
			...
			delay(RampDelay);					// Adjust this until smooth ramp is obtained
			// Serial.println(i);
			}
